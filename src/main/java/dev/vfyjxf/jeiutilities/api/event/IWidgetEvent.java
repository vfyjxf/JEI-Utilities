...
public interface IWidgetEvent {
    ...
    UIEventDefinition<OnMeasure> onMeasure = UIEventFactory.define(OnMeasure.class, listeners -> (context, size) -> {
        for (var listener : listeners) {
    ...
    UIEventDefinition<OnLayout> onLayout = UIEventFactory.define(OnLayout.class, listeners -> (context) -> {
        for (var listener : listeners) {
    ...
    interface OnPositionChanged extends IWidgetEvent {
        void onPositionChanged(IEventContext.Common context, Point position);
    }

    @FunctionalInterface
    interface OnSizeChanged extends IWidgetEvent {
        void onSizeChanged(IEventContext.Common context, Dimension size);
    }


    @FunctionalInterface
    interface OnRender extends IWidgetEvent {
        void onRender(Minecraft mc, int mouseX, int mouseY, float partialTicks, IEventContext.Common context);
    }
    ...
    interface OnChildAdded extends IWidgetEvent {
        void onChildAdded(IEventContext.Common context, Widget widget);
    }
    ...
    interface OnChildRemoved extends IWidgetEvent {
        void onChildRemoved(IEventContext.Common context, Widget widget);
    }
    ...
    interface OnTooltip extends IWidgetEvent {
        void onTooltip(IEventContext.Common context, List<String> tooltip);
    }

    interface OnMeasure extends IWidgetEvent {
        void onMeasure(IEventContext.Interruptible context, Dimension size);
    }

    interface OnLayout extends IWidgetEvent {
        void onLayout(IEventContext.Interruptible context);
    }
