...
public class RecipeHelper {
    ...
    public static <V, T> Pair<IRecipeWrapper, Integer> getRecipeWrapperAndIndex(
        ...
        if (recipeCategory != null) {
            IIngredientHelper<V> ingredientHelper = JeiUtilitiesPlugin.ingredientRegistry.getIngredientHelper(focus.getValue());
            IFocus<?> translatedFocus = ingredientHelper.translateFocus(focus, Focus::new);
            List<IRecipeWrapper> recipes = JeiUtilitiesPlugin.recipeRegistry.getRecipeWrappers(recipeCategory, translatedFocus);
            IIngredientType<T> outputType = JeiUtilitiesPlugin.ingredientRegistry.getIngredientType(recipeOutput);
    ...
    public static <V, T,R extends IRecipeWrapper> Pair<ICraftingRecipeWrapper, Integer> getCraftingRecipeWrapperAndIndex(
        ...
        IRecipeCategory<R> recipeCategory = JeiUtilitiesPlugin.recipeRegistry.getRecipeCategory(recipeCategoryUid);
        if (recipeCategory != null && recipeCategory.getUid().equals(VanillaRecipeCategoryUid.CRAFTING)) {
            ...
            List<ICraftingRecipeWrapper> craftingRecipes = JeiUtilitiesPlugin.recipeRegistry.getRecipeWrappers(recipeCategory, translatedFocus)
                    .stream()
            ...
            return registerNameMatchRecipes.parallelStream()
                    .filter(recipe -> {
                                Ingredients outputIngredients = new Ingredients();
                                recipe.getIngredients(outputIngredients);
                                List<List<T>> outputSlots = outputIngredients.getOutputs(JeiUtilitiesPlugin.ingredientRegistry.getIngredientType(recipeOutput));
                                return outputSlots.parallelStream()
                                        .flatMap(Collection::stream)
                                        .map(t -> IngredientHelper.getUniqueId(IngredientHelper.getNormalize(t)))
